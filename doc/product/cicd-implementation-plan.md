...
Both Docker jobs build the engineering-system's canonical `build/Dockerfile` (the action default per ADR-037) and pass `build_args: JAR_FILE=provider/<SERVICE_NAME>-azure/target/*-spring-boot.jar` (override via the `SERVICE_TARGET_JAR` repository variable) so the JAR our `java-build` job produced is the one copied into the image.
